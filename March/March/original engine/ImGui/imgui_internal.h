...
struct ImGuiPopupRef
{
    ...
    ImGuiWindow*        ParentWindow = nullptr;   // Set on OpenPopup()
    int                 OpenFrameCount = 0; // Set on OpenPopup()
    ...
    ImVec2              OpenPopupPos = {0,0};   // Set on OpenPopup(), preferred popup position (typically == OpenMousePos when using mouse)
    ImVec2              OpenMousePos = { 0,0 };   // Set on OpenPopup(), copy of mouse position at the time of opening popup
};

struct ImGuiColumnData
{
    float               OffsetNorm = 0;         // Column start offset, normalized 0.0 (far left) -> 1.0 (far right)
    float               OffsetNormBeforeResize = 0;
    ImGuiColumnsFlags   Flags = 0;              // Not exposed
    ImRect              ClipRect;
