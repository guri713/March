...
///</summary>
LRESULT CALLBACK fnWndProc(HWND _hwnd, UINT _msg, WPARAM _w_param, LPARAM _l_param)
{

#ifdef USE_IMGUI
	if (ImGui_ImplWin32_WndProcHandler(_hwnd, _msg, _w_param, _l_param))
		return 1;
#endif
...
bool Window::Init(HINSTANCE _instance)
{
	...
	//ウインドウの生成
	hwnd = CreateWindowExW(0L, setup.window_name.c_str(), setup.window_name.c_str(), (setup.window_mode == e_window_mode::window ? (WS_OVERLAPPEDWINDOW /*^ WS_MAXIMIZEBOX */ ^ WS_THICKFRAME | WS_VISIBLE) : (WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP)), _window_pos.x, _window_pos.y, setup.window_size.x, setup.window_size.y, NULL, NULL, _instance, NULL);
